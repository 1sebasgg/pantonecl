...
export function ExposicionesTimeline() {
  return (
    <section id="exposiciones-timeline" className="mx-auto max-w-4xl px-6 py-20 md:px-10 md:py-28">
      <p className="text-xs uppercase tracking-[0.28em] text-accent">Línea de tiempo</p>
      <h1 className="mt-4 font-display text-5xl font-semibold tracking-tight md:text-6xl">
        Exposiciones
      </h1>
      <p className="mt-6 max-w-2xl text-base leading-relaxed text-muted-foreground">
        Un recorrido cronológico por las muestras que hemos curado y producido, desde instalaciones
        internacionales hasta exposiciones patrimoniales.
      </p>
      ...
      <ol className="mt-16 border-l border-border">
        {timeline.map((item) => (
          <li key={item.year + item.title} className="relative pb-14 pl-8 last:pb-0">
            ...
            <p className="font-display text-3xl font-semibold tracking-tight">{item.year}</p>
